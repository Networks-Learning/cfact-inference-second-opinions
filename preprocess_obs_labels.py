...
experts = (np.sum(labels!=-999, axis=0) > 140) & (np.sum(labels_test!=-999, axis=0) > 20)
...
row_idx = np.any(labels!=-999, axis=1)
data = data[row_idx]
...
row_idx_test = np.sum(labels_test[:,clique]!=-999, axis=1)>1
...
has_disagreement = vec_has_disagreement(labels[row_idx][:, clique])
